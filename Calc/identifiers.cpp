...
template <const ParamCount> using WrappedFnImplArg = Value;

template <const ParamCount... Is>
consteval auto WrappedFnImpl(std::index_sequence<Is...>) noexcept
    -> Value (*)(WrappedFnImplArg<Is>...) {
...
function_pointer_impl(std::span<Value> params) noexcept {
...
solve(std::span<Value> params) noexcept {
  switch (params.size()) {
  case 3:
    return solve_quadratic(params[0], params[1], params[2]);
  case 4:
    return solve_cubic(params[0], params[1], params[2], params[3]);
  default:
    std::unreachable();
  };
}
...
static const map ids = {
    ...
    {"alpha_particle_mass_energy_MeV",
     constant<alpha_particle_mass_energy_MeV>()},
    ...
    {"neutron_proton_mass_difference",
     constant<neutron_proton_mass_difference>()},
    {"neutron_proton_mass_diff_energy_J",
     constant<neutron_proton_mass_diff_energy_J>()},
    {"neutron_proton_mass_diff_energy_MeV",
     constant<neutron_proton_mass_diff_energy_MeV>()},
    ...
    {"reduced_planck_constant", constant<reduced_planck_constant>()},
...
[[nodiscard]] const map &get() noexcept { return ids; }
