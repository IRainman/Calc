...
//#error "Because Calc is the Dialog based application version below Windows 2000 isn't supported."
#endif

#if(_WIN32_WINNT < _WIN32_WINNT_WINXP)
#warning "In Windows before Windows XP the link in the About box isn't working bacause system API isn't exist."
#endif

#if(_WIN32_WINNT < _WIN32_WINNT_WS03)
#warning "In Windows before Windows Server 2003 the max input is limited to 32767 symbols."
#endif
...
struct RegWrite {
    ...
    RegWrite(const HKEY root, const char* subkey) noexcept {
        RegCreateKeyExA(root, subkey, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &key, nullptr);
    }
    ~RegWrite() {
        RegCloseKey(key);
    }
...
struct RegRead {
    ...
    std::optional<DWORD> read(const char* name) const {
        DWORD type [[indeterminate]]; DWORD out [[indeterminate]]; DWORD outSize = sizeof(out);
        if (RegQueryValueExA(key, name, nullptr, &type, reinterpret_cast<LPBYTE>(&out), &outSize) == ERROR_SUCCESS) {
            if (type == REG_DWORD) {
                return out;
            }
        }
    ...
    DWORD read(const char* name, LPBYTE out, DWORD out_size) const {
        ...
        if (RegQueryValueExA(key, name, nullptr, &type, out, &out_size) == ERROR_SUCCESS) {
            if (type == REG_BINARY) {
                return out_size;
            }
        }
...
struct Point : tagPOINT {
    ...
    constexpr auto get_x() const noexcept {
        return x;
    }
    constexpr auto get_y() const noexcept {
        return y;
    }
};
...
struct Rect : tagRECT {
    Rect() = default;
    constexpr Rect(const LONG x, const LONG y, const LONG width, const LONG heigth) noexcept {
        left = x; right = x + width;
        top = y; bottom = y + heigth;
    }
    constexpr auto get_x() const noexcept {
        return left;
    }
    ...
    constexpr auto get_heigth() const noexcept {
        return bottom - top;
    }
};
using Rect_ptr = Rect*;
...
// ------------------------------------------------------------------
static void center_window_on_monitor(const HWND window, const HMONITOR monitor) {
    ...
    if (GetWindowRect(window, &wr)) {
        ...
        SetWindowPos(window, nullptr, CalcConfiguration::default_shift_px, CalcConfiguration::default_shift_px, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
    }
}
#endif
...
public:
    ...
    struct Constraint {
        constexpr void init(const Layout* layout, const HWND _handle, const Rect& client, const Anchor _anchor) noexcept {
            handle = _handle;
    ...
    void init_min_sizes(const HWND window, const LONG requested_min_width, const LONG requested_min_height
#ifdef CALC_SUPPORT_PER_WINDOW_DPI
        , UINT dpi
#endif
    ) {
        ...
        const auto exStyle = static_cast<DWORD>(GetWindowLongPtrA(window, GWL_EXSTYLE));
    ...
    void init_anchor(const HWND parent, const size_t index, const int id, const Anchor anchor) {
        ...
        const Rect client(client_point.get_x(), client_point.get_y(), window.get_width(), window.get_heigth());
    ...
    void resize(const LONG new_width, const LONG new_height) {
        ...
        for (auto& c : constraints) {
            Rect r [[indeterminate]];

            // Horizontal
            switch (c.anchor.h) {
            case HMode::Left:
                r.right = c.relative_margins.left + c.width;
                r.left = c.relative_margins.left;
                break;

            case HMode::Right:
                r.right = width - c.relative_margins.right;
            ...
            switch (c.anchor.v) {
            case VMode::Top:
                r.bottom = c.relative_margins.top + c.height;
                r.top = c.relative_margins.top;
                break;

            case VMode::Bottom:
                r.bottom = height - c.relative_margins.bottom;
    ...
    constexpr auto get_min_width() const {
        return min_width;
    }
    constexpr auto get_min_x() const {
        return min_width;
    }
    constexpr auto get_min_height() const {
        return min_height;
    }
    constexpr auto get_min_y() const {
        return min_height;
    }
    constexpr auto get_constraints_handle(const BYTE index) const {
        return constraints[index].handle;
    }
private:
    std::array<Constraint, CalcConfiguration::elements> constraints [[indeterminate]];
