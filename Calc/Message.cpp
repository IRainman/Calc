...
inline void AddMessage(const unsigned int p_message)
{
	...
	swprintf_s(l_temp_buf, L"AddMessage(%u... ", p_message);
...
#ifdef _DEBUG
	wchar_t l_temp_buf[1024];
	swprintf_s(l_temp_buf, L"AddWarning(%u... ", p_message);
	m_ErrorString += l_temp_buf;
...
#ifdef _DEBUG
	wchar_t l_temp_buf[1024];
	swprintf_s(l_temp_buf, L"AddError(%u... %s", p_message, l_out_buf); //-V111
	m_ErrorString += l_temp_buf;
